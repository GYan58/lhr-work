...
class CacheSim:
    ...
    def evict(self):
        SIDs = rd.sample(list(self.Caches.keys()),512)
...
class CacheSys:
    def __init__(self,Csize, AProb=0.0):
        ...
        if self.Warmup == True:
            ...
            if self.RefillSize >= self.CSize * 4 and self.LenCount >= 10000:
                ...
                if Train == True:
                    ...
                    self.LastTrained = False
                if Train == False:
                    if len(self.PastReqs) > self.WarmSize:
                        self.PastReqs = self.PastReqs[-self.WarmSize:]
